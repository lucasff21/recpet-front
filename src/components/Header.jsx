...
const Header = () => {
    ...
    const USER_MENU = [
        ...
        { action: () => { logout(); navigate('/'); }, label: 'Sair', icon: 'logout' }
    ];
    ...
    const renderUserMenu = (isMobile = false) => {
        ...
        return (
            <div className={isMobile ? "md:hidden" : "hidden md:block relative"}>
                ...
                >
                    <span className="font-bold">{user.nome.split(' ')[0] || user.email}</span>
                    <svg
                ...
                {userMenuOpen && (
                    <div className={`${isMobile ? 'ml-4 mt-2 space-y-2' : 'absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-10'}`}>
                        {renderUserMenuItems()}
                    </div>
                )}
            </div>
    ...
    return (
        <header className="flex justify-between items-center px-4 py-2 bg-white shadow-md relative">
            ...
            <nav className={`absolute z-2 gap-4 md:static top-16 left-0 w-full md:w-auto bg-white shadow-md md:shadow-none flex flex-col md:flex-row md:space-x-8 p-2 md:p-0 ${menuOpen ? 'block' : 'hidden'} md:flex`}>
                {LINKS.map((link, index) => (
                    <NavLink
                        key={index}
                        to={link.path}
                        className={({isActive}) => `no-underline font-bold hover:text-orange-600 ${isActive ? 'text-orange-500 font-bold hover:text-orange-600' : 'text-gray-700 hover:text-gray-900'}`}
