        ...
        <div className="relative mb-10">
            ...
            <div className="hidden md:flex justify-between relative">
                ...
                {steps.map((step) => (
                    ...
                    >
                        <span className={`w-8 h-8 rounded-full flex items-center justify-center mb-2 font-bold 
                            ${currentStep >= step.number ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-600'}`}>
                            {step.number}
                        </span>
                        <p className={`text-sm ${currentStep >= step.number ? 'text-green-600 font-medium' : 'text-gray-500'}`}>
                            {step.label}
                        </p>
                    </div>
                ))}
            ...
            <div className="md:hidden space-y-4">
                {steps.map((step) => (
                    <div
                        ...
                        onClick={() => goToStep(step.number)}
                    >
                        <span className={`w-8 h-8 rounded-full flex items-center justify-center mr-3 font-bold 
                            ${currentStep >= step.number ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-600'}`}>
                            {step.number}
                        </span>
                        <p className={`m-0 ${currentStep >= step.number ? 'text-green-600 font-medium' : 'text-gray-500'}`}>
                            {step.label}
                        </p>
                    </div>
                ))}
